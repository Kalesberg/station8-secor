...
export default ({ block, styles, images }) => {
    ...
    () => [
      {
        ...
        // tickValues: ['1', 0.2, 0.4, 0.6, 0.8, 1]
        // filterTicks: (ticks) =>
  ...
  const handleRotate = () => {
    ...
    setActive(active === enabled(block.features).length - 1 ? 0 : active + 1)
  }
      ...
      <div className={styles.infographic}>
        ...
        <div className={styles.list}>
          <div className={styles.listContainer}>
            {enabled(block.features).map((blend, i) => {
              return (
                <div key={i} className={styles.blend + `${i === active ? ` ${styles.active}` : ''}`}>
                  {enabled(blend.list).map((item, i) => {
                    return (
